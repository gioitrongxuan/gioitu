...
- [ ] **Sync trung thực**: syncUserData trả `{pulled, pushed}`; toast warn khi
  ...
  auth.ts:8,73`
- [ ] **Race di trú guest khi đăng nhập**: await `reassignEntries` TRƯỚC
  `setSession` (hiện MainApp remount + sync chạy đua với migrate → từ "biến
  mất" sau đăng nhập). `App.tsx:53-74`
- [ ] **Phân biệt lỗi mạng vs "không tìm thấy"**: serverDict/getJson nuốt mọi
  lỗi thành []/null → offline hiện "Không tìm thấy. Tự định nghĩa từ này:".
  Trả kiểu kết quả có cờ lỗi + thông điệp riêng + gợi ý chuyển nguồn. Tương tự
  handwritingApi (vẽ xong trống trơn không lời giải thích).
  `serverDict.ts:17-25 · DetailPanel.tsx:307-324 · handwritingApi.ts:8-22`
...
- [ ] Ease chỉ trừ trong REVIEW (hiện "applies regardless of phase" → ease hell
  từ trước khi được ôn thật). `srs.ts:73-89`
- [ ] Fuzz interval ±5–10% (thẻ add cùng ngày mãi due cùng ngày). `srs.ts:157-158`
- [ ] Trần interval (~365 ngày, trùng knownInterval). `srs.ts:142-158`
- [ ] Relapse/lapse khôi phục theo % interval cũ (hiện 365 ngày rơi thẳng về
  10 phút, tốt nghiệp lại từ 1 ngày). `srs.ts:191-212`
...
- [ ] Đếm "Đã thuộc (N)" thường trực (menu hiện chỉ xuất hiện khi N>0);
  ...
  LearnedCloud.tsx:66`
- [ ] Sync theo sự kiện: debounce sau loạt putEntry, khi đóng phiên ôn,
  visibilitychange (hiện chỉ lúc mở app + nút bấm → cả buổi ôn nằm local).
  `store.ts:27-35`
