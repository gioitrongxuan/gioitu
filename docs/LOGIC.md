...
leechLapseThreshold    = 8            ← số lần rớt coi thẻ là "leech" (chỉ cảnh báo, §4.6)
```
...
### 4.6 Leech — thẻ khó nhằn (`isLeech(entry, cfg)`)

Một thẻ là **leech** ("khó nhằn") khi số lần rớt tích luỹ đạt/vượt ngưỡng
`leechLapseThreshold` (mặc định **8**, theo Anki):

```
isLeech(entry, cfg) = entry.lapses >= cfg.leechLapseThreshold
```

Suy thẳng từ trường `lapses` **sẵn có** (tăng ở mỗi "Again" trong REVIEW — §4.3 —
và mỗi relapse-do-tra-cứu — §4.4) nên **không thêm trường model**. Đây chỉ là
**phát hiện** để UI hiện huy hiệu cảnh báo + gợi ý hành động (phiên ôn hiện nhãn
"Khó nhằn" kèm gợi ý sửa nghĩa/tạm gác — `ReviewSession.tsx`); engine **không** tự
hoãn/xoá thẻ và **không** đổi lịch biểu SM-2. Việc "tạm hoãn"/"đánh dấu xử lý
riêng" là quyết định mở, để người dùng chủ động.

### 4.7 Nhật ký ôn tập — `review_log` (append-only)
